...
export async function loadQuartzLayout(layoutOverrides?: {
  ...
  defaultLayout.footer = footer ? [footer] : (defaultLayout.footer ?? [])
  ...
  for (const pageType of Object.keys(byPageType)) {
    ...
    // MyFooter is the single footer source for every page type — override any
    // components that plugins may have auto-registered via manifest defaultPosition.
    pt.footer = footer ? [footer] : (pt.footer ?? defaultLayout.footer)
  }
